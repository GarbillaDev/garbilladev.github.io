...
const QUIZ = [
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateHUD() {
      ...
      unlockedList.innerHTML = unlocked.length ? unlocked.map(u => `<div>• ${u.title}</div>`).join('') : 'Nenhuma ainda';
    }
    ...
    function onChoose(idx, q) {
      ...
      if (correct) {
        ...
        typeWriteTo(out, `> Resposta correta ✓\n${q.card.title}: ${q.card.text}`).then(() => { nextBtn.disabled = false; updateHUD(); });
      } else {
        score = Math.max(0, score - 60);
        appendHistory(`-60 — erro em "${q.id}"`);
        S.beep(220, 0.18, 0.06, 'sawtooth');
        typeWriteTo(out, `> Resposta incorreta ✕\nResposta correta: ${q.options[q.ans]}`).then(() => { nextBtn.disabled = false; updateHUD(); });
      }
    }
    ...
    startBtn.addEventListener('click', () => {
      levelIdx = 0; qIdx = 0; score = 0; unlocked = []; historyEl.innerHTML = ''; toolCard.innerHTML = 'Nenhuma ainda';
      running = true; updateHUD();
      typeWriteTo(out, 'Iniciando SimHack — Quiz educativo...\n', 10).then(() => {
        loadQuestion();
      });
    });
    nextBtn.addEventListener('click', () => { if (running) next(); });
    resetBtn.addEventListener('click', () => {
      running = false; levelIdx = 0; qIdx = 0; score = 0; unlocked = []; updateHUD();
      out.innerHTML = 'Quiz reiniciado. Clique em Iniciar para começar.';
      choicesEl.innerHTML = ''; toolCard.innerHTML = 'Nenhuma ainda'; historyEl.innerHTML = '';
      nextBtn.disabled = true;
    });
